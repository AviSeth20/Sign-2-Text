...
# Configure TensorFlow to be quieter
...
class Application:
    ...
    def __init__(self):
        ...
        # Disable the verbose output for all model predictions
        '''self.loaded_model._make_predict_function()
        self.loaded_model_dru._make_predict_function()
        self.loaded_model_tkdi._make_predict_function()
        self.loaded_model_smn._make_predict_function()
        '''
    ...
    def video_loop(self):
        ...
        if ok:
            ...
            # Changed from Hunspell to enchant for suggestions
            if self.word:
                predicts = self.dictionary.suggest(self.word)
            else:
                predicts = []
    ...
    def predict(self, test_image):
        # Suppress output from TensorFlow during prediction
        with tf.keras.utils.custom_object_scope({}):
            test_image = cv2.resize(test_image, (128, 128))
            
            # Run model predictions with verbose=0 to suppress output
